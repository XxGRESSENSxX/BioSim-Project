...
# --- 3. SIDEBAR (RESTRUTURADA CONFORME PEDIDO) ---
with st.sidebar:
    ...
    # OS DOIS SLIDERS SEPARADOS (RESTAURADOS)
    gordura = st.slider("Gordura Corporal (BF %)", 5, 50, 22) # VOLTOU
    massa_magra = st.slider("Massa Magra (%)", 10, 90, 40)
    
    st.divider()
    st.subheader("📋 Histórico Clínico")
    # CAMPOS SEPARADOS (RESTAURADOS)
    estilo_vida = st.text_input("Estilo de Vida:", placeholder="Ex: Maratonista, fumante...") # VOLTOU SEPARADO
    patologias = st.multiselect(
        "Patologias/Comorbidades Prévia:",
        ["Diabetes", "ICC", "Hipertensão", "DPOC", "Asma", "Insuficiência Renal"],
        default=[]
    )
...
if c_bt.button("EXECUTAR SIMULAÇÃO") and droga:
    with st.spinner("Analisando cascata bioquímica..."):
        # Prompt completo com TODOS os parâmetros separados
        ...
        try:
            ...
            if match:
                ...
                st.rerun() # Atualiza os números no topo na hora
        except Exception as e:
            st.error("Erro de cota ou conexão. Aguarde 1 minuto.")
